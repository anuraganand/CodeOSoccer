...
void CGraphics::drawHollowCircle(float x0,float y0,float xr, float yr, float epsilon)
{
    glBegin(GL_LINE_LOOP);
    for(float angle = 0.0f; angle <= 2.0 * M_PI; angle += (M_PI/20.0f))
    {
          glVertex3f(x0 + xr * cos(angle), y0 + yr * sin(angle), epsilon);
    }
    glEnd();
}
