...
#include "proto/sslDebug_Data.pb.h"
...
{    
    ...
    for (int k=0;k<5;k++)
        robots[k] = new Robot(p,ball,cfg,-form1->x[k],form1->y[k]+((k>=3)?10:0),ROBOT_START_Z(cfg),ROBOT_GRAY,ROBOT_GRAY,ROBOT_GRAY,k+1,wheeltexid,1);
    cfg->robotSettings = cfg->yellowSettings;
    for (int k=0;k<5;k++)
        robots[k+5] = new Robot(p,ball,cfg,form2->x[k],form2->y[k]+((k>=3)?10:0),ROBOT_START_Z(cfg),ROBOT_GRAY,ROBOT_GRAY,ROBOT_GRAY,k+6,wheeltexid,-1);
...
void SSLWorld::sendVisionBuffer()
{
    ...
    while (t - sendQueue.front()->t>=cfg->sendDelay())
    {
        ...
        delete packet;
  qDebug()<<"Packet sent frame Number: "<< packet->detection().frame_number()<<"\n";
